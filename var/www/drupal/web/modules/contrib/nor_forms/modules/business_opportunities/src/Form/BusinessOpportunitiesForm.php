...
namespace Drupal\business_opportunities\Form;
...
class BusinessOpportunitiesForm extends FormBase
{

  public function getFormId()
  {
    return 'business_opportunities_form';
  }

  public function buildForm(array $form, FormStateInterface $form_state)
  {

    $form['#prefix'] = '<div id="business-container" div class="business-container">';


    $form['header'] = [
      '#type' => 'markup',
      '#markup' => '<h2>Business Opportunities</h2><p>Please submit the following form and a representative will follow up with you shortly.</p>',
    ];
    ...
    $form['business_attachment'] = [
      '#type' => 'managed_file',
      '#title' => $this->t('Attach a File (pdf, png, jpg, jpeg)'),
      '#upload_validators' => [
        'file_validate_extensions' => ['png jpeg jpg pdf'],
      ],
      /* '#upload_location' => 'public://business_opportunities', */
      '#upload_location' => 'temporary://business_opportunities',
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state){
    ...
    $first_name = $form_state->getValue('business_fname');
    $last_name = $form_state->getValue('business_lname');
    $attachment = $form_state->getValue('business_attachment');

    if (empty($first_name) || strlen($first_name) < 2) {
      $form_state->setErrorByName('business_fname', $this->t('Please enter your first name.'));
    }

    if (empty($last_name) || strlen($last_name) < 2) {
      $form_state->setErrorByName('business_lname', $this->t('Please enter your last name.'));
    }

    if ($first_name && $last_name && $first_name === $last_name) {
      $form_state->setErrorByName('business_lname', $this->t('Last name should not be the same as first name.'));
    }

    if ($first_name && $last_name && strlen($last_name) >= 6 && strpos($last_name, $first_name) !== false) {
        $form_state->setErrorByName('business_lname', $this->t('Last name should not contain first name for 6 or more characters.'));
    }
    
    if($attachment){
      $form_state->set('file_id', $form_state->getValue('business_attachment')[0]); // save temp file fid
    }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    $first_name = $form_state->getValue('business_fname');
    $last_name = $form_state->getValue('business_lname');
    $email = $form_state->getValue('business_email');
    $country_code = $form_state->getValue('business_country');
    $phone = $form_state->getValue('business_phone');
    $company = $form_state->getValue('business_company');
    $message = $form_state->getValue('business_message');
    $date = date("Ymd");
    $form_name = $this->t('Business Opportunities Form');
    $output = "";
    $country_name = getCountryNames($country_code);

    $moved_file = null;
    $email_dir_name = nor_forms_email_to_directory_name($email);
    $fid = $form_state->get('file_id') ?? $form_state->getValue('business_attachment')[0];
    if($fid){
      $file = File::load($fid);
      $permanent_uri = 'private://business_opportunities/' .$email_dir_name . '/' . date('Y-m-d'); // uses email and date to store the files. E.g. liam.howes@norgenbiotek.com submitting on May 23 2025 saves to: private://business_opportunities/liam_howes_norgenbiotek_com_4901bb87/2025-05-23
    ...
    $query->fields(['created_on', 'first_name', 'last_name', 'company', 'country', 'email', 'record', 'timestamp', 'form_name', 'phone', 'notes', 'doc_fid']); 
    ...
    try {
      ...
      $record = [
        ...
        'Web_Forms' => ['Business Opportunities'],
      ];
    ...
    $output = '<p>Hello,</p>
    <p>A customer submitted their request for Business Opportunities.</p>
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $subject = '[Contact form] - Business Opportunities ' . date("F j, Y, g:i a", $time);
