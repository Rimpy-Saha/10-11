...
namespace Drupal\social_media_popup\Form;
...
class SocialMediaPopupForm extends FormBase
{

  public function getFormId()
  {
    return 'social_media_popup_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state)
  {
    ...
    //   '#required' => TRUE,
    //   '#attributes' => [
    //     'placeholder' => $this->t('First Name(required)'), // Placeholder text
    //   ],
    // ];
    ...
    $form['newsletter_lname'] = [
      ...
      '#required' => TRUE,
      '#attributes' => [
        'placeholder' => $this->t('Name(required)'), // Placeholder text
        'autocomplete'=> "name",
      ],
    ];
    ...
    $form['newsletter_email'] = [
      ...
      '#required' => TRUE,
      '#attributes' => [
        'placeholder' => $this->t('Email Address(required)'), // Placeholder text
      ],
    ];
    ...
    $form['newsletter_job'] = [
      '#type' => 'textfield',
      '#required' => TRUE,
      '#default_value' => nor_forms_user_job_title(),
      '#attributes' => [
        'placeholder' => $this->t('Job Title(required)'), // Placeholder text
        'autocomplete'=> "orginization-title",
      ],
    ];
    // $form['newsletter_country'] = [
    //   '#type' => 'textfield',
    //   '#required' => TRUE,
    //   // '#default_value' => commerce_nor_cart_popup_get_user_country(), // Get the user's country dynamically
  ...
  public function validateForm(array &$form, FormStateInterface $form_state)
  {
    ...
    if (empty($last_name) || strlen($last_name) < 2) {
      $form_state->setErrorByName('newsletter_lname', $this->t('Please enter your name.'));
    }
  ...
  public function submitCallback(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $response = nor_forms_email_sent_social_media_newsletter_ajax($form, $form_state, $Selector);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    $utm_medium = $form_state->getValue('utm_medium', 'direct_visited');
    $referrer_page = $form_state->getValue('referrer_page', 'No Referrer (Direct Visit)');

    $first_name = $form_state->getValue('newsletter_fname');
    $last_name = $form_state->getValue('newsletter_lname');  // FULL NAME
    $email = $form_state->getValue('newsletter_email');

    // $phone = $form_state->getValue('newsletter_phone');
    $job_title = $form_state->getValue('newsletter_job');
    ...
    $date = date("Ymd");
    $form_name = $this->t('Social Media Popup Form');
    ...
    <p>Name: ' . $last_name . '<br>Email: ' . $email . '<br>Job Title: ' . $job_title . '<br>Lead Source: ' . $utm_source . '<br>UTM Medium: ' . $utm_medium . '<br> Referrer Page: ' . $referrer_page . '<br> Country: ' .$country_name;

    $time = time();
    $recipient_emails = 'sabah.butt@norgenbiotek.com'; // Default recipient
    if (!empty($utm_source) || !empty($utm_medium)) {
      $recipient_emails .= ', info@norgenbiotek.com'; // Add second recipient only if all fields are filled

      $query = \Drupal::database()->insert('forms_to_zoho');
      $query->fields(['created_on','last_name', 'email', 'record', 'timestamp', 'form_name', 'job_title', 'country', 'lead_source', 'opt_in']); //wrong syntax here breaks entire submit function 
      $query->values([$date, $last_name, $email, '', time(), $form_name, $job_title , $country_name, $utm_source , 1]);
      $query->execute();

      try {  //Zoho upsert
        $zoho = new RecordWrapper('contacts');
        $record = [
          'Last_Name' => $last_name,
          'Email' => $email,
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $recipient_email = 'emarketing@norgenbiotek.com';
      //$recipient_email = 'huraira.khan@norgenbiotek.com';
      //$recipient_emails = 'liam.howes@norgenbiotek.com';
      nor_forms_email_redirect($output, $recipient_emails, $subject);
    }
  }
}
