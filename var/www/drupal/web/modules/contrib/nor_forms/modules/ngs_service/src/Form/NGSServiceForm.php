...
class ngsserviceform extends FormBase
{

  public function getFormId() {
    return 'ngs_service_form';
  }

  public function buildForm(array $form, FormStateInterface $form_state) {

    $form['#prefix'] = '<div id="ngsserviceform-container" class="ngsserviceform-container">';

    $form['header'] = [
      '#type' => 'markup',
      '#markup' => '<h2>NGS Service Form</h2><p>Please submit the following form and you will gain access to the following resource.</p>',
      '#class' => 'work_pls',
    ];
    ...
    $form['account_message'] = [
      ...
      '#type' => 'textarea',
      '#description' => 'Please provide details regarding your inquiry.',
    ...
    $form['account_attachment'] = [
      ...
      '#upload_location' => 'public://ngs_service',
    ];
  ...
  public function submitCallback(array &$form, FormStateInterface $form_state) {
    $Selector = '#ngsserviceform-container';
    $response = new AjaxResponse();
    
    // Get the current url
    $current_uri = \Drupal::request()->getRequestUri();  // e.g., "/content/ngs-services-small-rna-seq"
    ...
    if ($form_state->hasAnyErrors()) {
        ...
        if (strpos($current_uri, '/services/rna-sequencing') !== false) {
            $response = nor_forms_download_rna_resource_ajax($form, $form_state, $Selector);
        } else {
          $response = nor_forms_download_resource_ajax($form, $form_state, $Selector);
        }
    }
    return $response;
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    ...
    $date = date("Ymd");
    $form_name = $this->t('NGS Service Form');
    ...
    if($fid){
      $file = File::load($fid);
      $permanent_uri = 'private://ngs_service/' .$email_dir_name . '/' . date('Y-m-d'); // uses email and date to store the files. E.g. liam.howes@norgenbiotek.com submitting on May 23 2025 saves to: private://ngs_service/liam_howes_norgenbiotek_com_4901bb87/2025-05-23
    ...
    $query->fields(['created_on', 'first_name', 'last_name', 'country', 'company', 'email', 'record', 'timestamp', 'form_name', 'phone', 'notes', 'doc_fid']); //wrong syntax here breaks entire submit function 
    $query->values([$date, $first_name, $last_name, $country_name, $company, $email, '', time(), $form_name, $phone, $message, $new_fid]);
    $query->execute();
    ...
    $output = '<p>Hello,</p>
    <p>A customer submitted their request for NGS Service Form.</p>
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $subject = '[Contact form] - NGS Service Form ' . date("F j, Y, g:i a", $time);
