...
namespace Drupal\stool_dna_automation_scripts_test\Form;
...
class StoolDnaAutomationScriptsFormTest extends FormBase
{

  public function getFormId()
  {
    return 'stool_dna_automation_scripts_test';
  }

  public function buildForm(array $form, FormStateInterface $form_state)
  {
    $form['#prefix'] = '<div id="stool-dna-automation-scripts-container" class="stool-dna-automation-scripts-container">';
    ...
    $form['stool_sample_type_fieldset'] = [
      '#type' => 'fieldset',
      '#attributes' => array(
        'class' => array('sample-type-fieldset'),
      ),
      '#prefix' => '<div id="stool-sample-type-wrapper">',
      '#suffix' => '</div>',
    ];


    $stool_sample_options = array(
      t('animal cells'),
      t('animal tissues'),
      t('blood'),
    ...
    $form['stool_sample_type_fieldset']['stool_sample_type'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('What Sample Types Do You Work With? (check all that apply)'),
      '#options' => array_combine($stool_sample_options, $stool_sample_options),
      '#ajax' => [
        'callback' => '::sampleTypeCallback',
        'disable-refocus' => FALSE, // Or TRUE to prevent re-focusing on the triggering element.
        'event' => 'change',
        'wrapper' => 'stool-sample-type-wrapper', // This element is updated with this AJAX callback.
      ],
      '#required' => TRUE,
    ];

    if ($form_state->getValue('stool_sample_type') !== null && in_array('Other', $form_state->getValue('stool_sample_type'))) {
      // Get the text of the selected option.
      $form['stool_sample_type_fieldset']['stool_sample_type_specify_other'] = [
        '#type' => 'textfield',
  ...
  public function validateForm(array &$form, FormStateInterface $form_state){
    ...
    if (empty($form_state->getValue('stool_sample_type'))) {
      $form_state->setErrorByName('stool_sample_type', $this->t('Please select your sample types.'));
    }
  ...
  {
    return $form['stool_sample_type_fieldset'];
  }

  public function submitCallback(array &$form, FormStateInterface $form_state){

    $Selector = '#stool-dna-automation-scripts-container';
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $filename = 'https://norgenbiotek.com/sites/default/files/gated_content/Stool%20DNA%20Automation%20Scripts.zip'; // default
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    $country_code = $form_state->getValue('stool_country');

    $stool_sample_type_specified_other = $form_state->getValue('stool_sample_type_specify_other');
    if($form_state->getValue('stool_sample_type_specify_other') !== null){
      $form_state->getValue('stool_sample_type')['Other'] = $stool_sample_type_specified_other; // replace "Other" value with the specified text value from the textfield
    }
    $stool_sample_type = implode(",", array_filter($form_state->getValue('stool_sample_type')));
    ...
    $subscribe = $form_state->getValue('stool_mailing') ? 1 : 0;

    // Get the value of the "Subscribe to Mailing List" checkbox for email
    $subscribe_mail = $form_state->getValue('stool_mailing') ? 'Yes' : 'No';
    ...
    $query->values([$date, $first_name, $last_name, $email, $stool_sample_type, '', time(), $form_name, $subscribe, $country_name]);
    $query->execute();
    ...
    <p>Last name: ' . $last_name . '<br>First name: ' . $first_name . '<br>Email: ' . $email . '<br>Sample Type: ' . $stool_sample_type . '<br>Country: ' . $country_name . ' <br>Mailing List: ' . $subscribe_mail;
