...
namespace Drupal\urine_preservation_login_form\Form;
...
class UrinePreservationLoginForm extends FormBase{

  public function getFormID(){
    return 'database_page_form';
  }

  private $valid_access_codes = ['ILOVEURINE'];
  private $gated_node_id = 2920;

  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'core/drupal.ajax';
    $form['#attached']['library'][] = 'core/jquery.form';
    $form['#attached']['library'][] = 'urine_preservation_login_form/urine_preservation_login_form';

    if (\Drupal::request()->cookies->has('urine_preservation_access_granted')) { // skip logging in + redirect if they've already submitted the form
      return $this->redirect('entity.node.canonical', ['node' => 2920]);
    }
    ...
    $form['#prefix'] = '<div id="urine-preservation-login-container" class="accountissuesform-container">';
    ...
    $form['top']['title'] = [
      '#type' => 'markup',
      '#markup' => '<h1 class="center">Urine Preservation Data Portal</h1>',
    ];
    ...
    $form['top']['description'] = [
      '#type' => 'markup',
      '#markup' => "<p>Our Data Portal is exclusive to select individuals.</p><p>Wan't access? <a href='/contact'>Contact us</a> to request an access code.</p>",
    ];
    ...
    $form['form_actions']['submit'] = [
      ...
      '#ajax' => [
        ...
        'wrapper' => 'urine-preservation-login-container',
        //'event' => 'click',
        'method' => 'append', 
        'effect' => 'fade',
      ],
  ...
  public function submitCallback(array &$form, FormStateInterface $form_state) {
    /* This works! */
    $Selector = '#urine-preservation-login-container';
    /* $content = [
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $response->addCommand(new RedirectCommand(Url::fromRoute('entity.node.canonical', ['node' => 2920])->toString()));
  ...
  public function submitForm(array &$form, FormStateInterface $form_state){
    ...
    $email = $form_state->getValue('email');
    $company = $form_state->getValue('company');
    $form_name = $this->t('Urine Data Room Access Form');
    $date = date("Ymd");

    setcookie('urine_preservation_access_granted', true, time() + 86400, '/', '', TRUE, TRUE);
