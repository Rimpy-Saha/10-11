...
namespace Drupal\plant_dna_automation_scripts_test\Form;
...
class PlantDnaAutomationScriptsFormTest extends FormBase
{

  public function getFormId()
  {
    return 'plant_dna_automation_scripts_test';
  }

  public function buildForm(array $form, FormStateInterface $form_state)
  {
    $form['#prefix'] = '<div id="plant-dna-automation-scripts-container" class="plant-dna-automation-scripts-container">';
    ...
    $form['plant_sample_type_fieldset'] = [
      '#type' => 'fieldset',
      '#attributes' => array(
        'class' => array('sample-type-fieldset'),
      ),
      '#prefix' => '<div id="plant-sample-type-wrapper">',
      '#suffix' => '</div>',
    ];


    $plant_sample_options = array(
      t('animal cells'),
      t('animal tissues'),
      t('blood'),
    ...
    $form['plant_sample_type_fieldset']['plant_sample_type'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('What Sample Types Do You Work With? (check all that apply)'),
      '#options' => array_combine($plant_sample_options, $plant_sample_options),
      '#ajax' => [
        'callback' => '::sampleTypeCallback',
        'disable-refocus' => FALSE, // Or TRUE to prevent re-focusing on the triggering element.
        'event' => 'change',
        'wrapper' => 'plant-sample-type-wrapper', // This element is updated with this AJAX callback.
      ],
      '#required' => TRUE,
    ];

    if ($form_state->getValue('plant_sample_type') !== null && in_array('Other', $form_state->getValue('plant_sample_type'))) {
      // Get the text of the selected option.
      $form['plant_sample_type_fieldset']['plant_sample_type_specify_other'] = [
        '#type' => 'textfield',
  ...
  public function validateForm(array &$form, FormStateInterface $form_state){
    ...
    $first_name = $form_state->getValue('plant_fname');
    $last_name = $form_state->getValue('plant_lname');

    if (empty($first_name) || strlen($first_name) < 2) {
      $form_state->setErrorByName('plant_fname', $this->t('Please enter your first name.'));
    }

    if (empty($last_name) || strlen($last_name) < 2) {
      $form_state->setErrorByName('plant_lname', $this->t('Please enter your last name.'));
    }
    if ($first_name && $last_name && $first_name === $last_name) {
      $form_state->setErrorByName('plant_lname', $this->t('Last name should not be the same as first name.'));
    }
    if ($first_name && $last_name && strlen($last_name) >= 6 && strpos($last_name, $first_name) !== false) {
      $form_state->setErrorByName('plant_lname', $this->t('Last name should not contain first name for 6 or more characters.'));
    }

    if (empty($form_state->getValue('plant_sample_type'))) {
      $form_state->setErrorByName('plant_sample_type', $this->t('Please select your sample types.'));
    }
  ...
  {
    return $form['plant_sample_type_fieldset'];
  }

  public function submitCallback(array &$form, FormStateInterface $form_state){

    $Selector = '#plant-dna-automation-scripts-container';
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      $filename = 'https://norgenbiotek.com/sites/default/files/gated_content/Plant%20DNA%20Automation%20Scripts.zip'; // default
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    $first_name = $form_state->getValue('plant_fname');
    $last_name = $form_state->getValue('plant_lname');
    $email = $form_state->getValue('plant_email');
    /* $company = $form_state->getValue('plant_company'); */
    $country_code = $form_state->getValue('plant_country');

    $plant_sample_type_specified_other = $form_state->getValue('plant_sample_type_specify_other');
    if($form_state->getValue('plant_sample_type_specify_other') !== null){
      $form_state->getValue('plant_sample_type')['Other'] = $plant_sample_type_specified_other; // replace "Other" value with the specified text value from the textfield
    }
    $plant_sample_type = implode(",", array_filter($form_state->getValue('plant_sample_type')));
    ...
    $subscribe = $form_state->getValue('plant_mailing') ? 1 : 0;

    // Get the value of the "Subscribe to Mailing List" checkbox for email
    $subscribe_mail = $form_state->getValue('plant_mailing') ? 'Yes' : 'No';
    ...
    $query->values([$date, $first_name, $last_name, $email, $plant_sample_type, '', time(), $form_name, $subscribe, $country_name]);
    $query->execute();
    ...
    <p>Last name: ' . $last_name . '<br>First name: ' . $first_name . '<br>Email: ' . $email . '<br>Sample Type: ' . $plant_sample_type . '<br>Country: ' . $country_name . ' <br>Mailing List: ' . $subscribe_mail;
